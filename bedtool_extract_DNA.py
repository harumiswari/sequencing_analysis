...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 21 20:19:43 2024
...
for barcode in range(11, 21):  # This now correctly processes barcodes 31 to 40
    csv_filepath = f'/Users/gwisna/Desktop/hF9mousecombined/BC{barcode}/BC{barcode}_CM001000.3_reads_counts.csv'
...
#############To filter annotated file for Ckm reads gene#############

def filter_by_gene_id_and_type(file_path, output_file, target_gene_id):
    # Load the data
    data = pd.read_csv(file_path, sep='\t', header=None)
    
    # Define a function to extract the gene_id from the complex attribute column
    def extract_gene_id(info_string):
        parts = info_string.split(';')
        gene_id_part = [part for part in parts if 'gene_id' in part]
        if gene_id_part:
            return gene_id_part[0].split('"')[1]  # Extract the gene ID from the string
        return None
    
    # Apply the function to the appropriate column to extract gene IDs
    data['gene_id'] = data[12].apply(extract_gene_id)  # Ensure this is the correct column for your data
    
    # Filter the data to include only entries that are of type 'gene' in column 7 and have the specified gene ID
    filtered_data = data[(data[6] == 'gene') & (data['gene_id'] == target_gene_id)]
    
    filtered_data = filtered_data.drop(columns=['gene_id'])
    
    # Save the filtered data
    filtered_data.to_csv(output_file, sep='\t', index=False, header=False)
    
    print(f"Filtered data for {output_file} saved.")

for barcode in range(11, 21):
    file_path = f'/Users/gwisna/Desktop/hF9mousecombined/BC{barcode}/BC{barcode}_annotated.txt'
    output_file = f'/Users/gwisna/Desktop/hF9mousecombined/BC{barcode}/BC{barcode}_annotated_filtered.txt'
    filter_by_gene_id_and_type(file_path, output_file, "ENSMUSG00000030399")
